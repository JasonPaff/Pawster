  ...
  return (
    <div>
      <Listbox as="div" value={selectedService} onChange={handleChange}>
        {({ open }) => (
          <>
            {/* <Listbox.Label className="text-sm font-medium text-gray-700 ">Services</Listbox.Label> */}
            <div className="relative">
              <span className="inline-block w-full">
                <Listbox.Button
                  as="div"
                  className="pl-3 py-2 w-full text-left focus:outline-none focus:shadow-outline-blue focus:border-accent-green relative border shadow-sm border-gray-300 rounded text-gray-800 bg-white cursor-pointer"
                >
                  <span className="block truncate">{selectedService}</span>
                  <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none text-2xl">
                    <HiSelector />
                  </span>
                </Listbox.Button>
              </span>
              <Transition show={open} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
                <Listbox.Options static className="border absolute w-full bg-white border-gray-300 rounded mt-1 z-10 focus:outline-none focus:border-accent-green shadow-lg">
                  {services.map((service) => (
                    <Listbox.Option key={service.name} value={service}>
                      {({ selected, active }) => (
                        <div className={`${active ? "text-white bg-green-600" : "text-gray-900"} cursor-default select-none relative py-2 pl-10 pr-4`}>
                          <span className={`${selected ? "font-semibold" : "font-normal"}`}>{service.label}</span>

                          {selected && (
                            <span className={`${active ? "text-white" : "text-green-600"} absolute inset-y-0 left-0 flex items-center pl-2`}>
