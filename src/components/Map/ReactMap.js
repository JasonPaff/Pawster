...
function ReactMap(props) {
  ...
  async function load() {
    ...
    if (hostAddresses.length > 0) setLoaded(true);
  }
    ...
    <>
      {loaded && (
        ...
        >
          {markers}
          {popupInfo && (
            <Popup anchor="top" longitude={Number(popupInfo.lng)} latitude={Number(popupInfo.lat)} closeOnClick={false} onClose={() => setPopupInfo(null)}>
              <div>
                <div className="mt-5">
