...
export default function SendMessage(props) {
  ...
  return (
    <div className="flex flex-col py-2  justify-center px-4">
      <form onSubmit={(e) => handleSendMessage(e)}>
        <input required type="text" placeholder="Subject" ref={subjectRef} onChange={(e) => setSubject(e.target.value)} className="shadow-sm block w-full sm:text-sm rounded-md" />
        <textarea
          required
          rows={4}
          placeholder="Message"
          ...
          className="shadow-sm mb-1 block w-full
                        sm:text-sm border rounded-md my-3 "
        />
        <div className="flex flex-row">
          <button type="submit" className="cursor-pointer w-full p-1 my-2">
            Send
          </button>
        </div>
      </form>
